...
function RecordPaymentModal({
  ...
  sub: Subscription; schoolId: number; onClose: () => void; onSaved: () => void;
}) {
  const recordFn = useServerFn(recordSubscriptionPayment);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm">
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          <p className="text-xs text-slate-500 bg-slate-50 rounded-xl px-3 py-2.5">
            This records a manual payment (e.g. bank transfer, Razorpay link). The subscription period will advance by one {sub.billingCycle?.replace("ly","") ?? "month"}.
          </p>
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Amount (₹)</label>
            <input type="number" value={f.amount} onChange={(e) => set("amount", e.target.value)} className={inputCls} min="1" required />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Payment date</label>
            <input type="date" value={f.paidAt} onChange={(e) => set("paidAt", e.target.value)} className={inputCls} required />
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5">Notes (optional)</label>
            <input value={f.notes} onChange={(e) => set("notes", e.target.value)} placeholder="e.g. Paid via bank transfer ref #123" className={inputCls} />
          </div>
          {error && (
            <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 rounded-xl px-3 py-2.5 text-sm">
              <AlertCircle className="w-4 h-4 shrink-0" /> {error}
          ...
          <div className="flex justify-end gap-3 pt-1">
            <button type="button" onClick={onClose} className="px-4 py-2.5 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 text-sm font-medium transition">Cancel</button>
            <button type="submit" disabled={saving} className="px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white text-sm font-bold transition">
...
function SchoolDetailPage() {
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="flex gap-2">
                ...
                {subscription && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 transition"
                  >
      ...
      {recordPayOpen && subscription && (
        <RecordPaymentModal
          sub={subscription}
          schoolId={schoolId}
          onClose={() => setRecordPayOpen(false)}
          onSaved={() => { setRecordPayOpen(false); load(); }}
        />
      )}
    </div>
  );
