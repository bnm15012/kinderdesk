CREATE TABLE `announcement_dismissals` (
	...
	CONSTRAINT `announcement_dismissals_user_announcement` UNIQUE(`user_id`,`announcement_id`)
);
...
CREATE TABLE `attendance_sessions` (
	...
	CONSTRAINT `attendance_sessions_class_date` UNIQUE(`class_id`,`date`)
);
--> statement-breakpoint
CREATE TABLE `class_enrollments` (
	...
	CONSTRAINT `class_enrollments_student_class_year` UNIQUE(`student_id`,`class_id`,`academic_year`)
);
--> statement-breakpoint
...
CREATE TABLE `classes` (
	...
	CONSTRAINT `classes_school_location_name` UNIQUE(`school_id`,`location_id`,`name`)
);
--> statement-breakpoint
...
CREATE TABLE `documents` (
	...
	`student_id` int,
	`staff_id` int,
	`type` enum('birth_certificate','immunization_record','aadhar_card','photo','background_check','other') NOT NULL,
	`r2_key` varchar(500),
...
CREATE TABLE `emergency_contacts` (
	...
	CONSTRAINT `emergency_contacts_student_phone` UNIQUE(`student_id`,`phone`)
);
--> statement-breakpoint
...
CREATE TABLE `fee_structures` (
	...
	CONSTRAINT `fee_structures_school_location_class_name` UNIQUE(`school_id`,`location_id`,`class_id`,`name`)
);
--> statement-breakpoint
...
CREATE TABLE `invoices` (
	...
	CONSTRAINT `invoices_school_student_fee_month` UNIQUE(`school_id`,`student_id`,`fee_structure_id`,`generated_month`)
);
--> statement-breakpoint
CREATE TABLE `locations` (
	...
	CONSTRAINT `locations_school_name` UNIQUE(`school_id`,`name`)
);
--> statement-breakpoint
...
CREATE TABLE `parents` (
	...
	CONSTRAINT `parents_student_email` UNIQUE(`student_id`,`email`)
);
--> statement-breakpoint
CREATE TABLE `payments` (
	...
	CONSTRAINT `payments_razorpay_id` UNIQUE(`razorpay_payment_id`)
);
--> statement-breakpoint
CREATE TABLE `plans` (
	`id` int AUTO_INCREMENT NOT NULL,
...
CREATE TABLE `staff` (
	...
	CONSTRAINT `staff_school_email` UNIQUE(`school_id`,`email`)
);
--> statement-breakpoint
CREATE TABLE `staff_attendance` (
	...
	CONSTRAINT `staff_attendance_staff_date` UNIQUE(`staff_id`,`date`)
);
--> statement-breakpoint
CREATE TABLE `staff_class_assignments` (
	...
	CONSTRAINT `staff_class_assignments_staff_class` UNIQUE(`staff_id`,`class_id`,`academic_year`)
);
--> statement-breakpoint
CREATE TABLE `staff_payroll` (
	...
	CONSTRAINT `staff_payroll_staff_month` UNIQUE(`staff_id`,`month`)
);
--> statement-breakpoint
CREATE TABLE `student_attendance` (
	...
	CONSTRAINT `student_attendance_student_date` UNIQUE(`student_id`,`date`)
);
--> statement-breakpoint
...
CREATE TABLE `students` (
	...
	CONSTRAINT `students_school_admission_number` UNIQUE(`school_id`,`admission_number`)
);
--> statement-breakpoint
...
CREATE TABLE `users` (
	...
	`phone` varchar(50),
	`role` enum('super_admin','school_admin','location_admin','teacher','staff','receptionist','parent','accountant') DEFAULT 'staff',
...
CREATE TABLE `waitlist` (
	...
	CONSTRAINT `waitlist_inquiry` UNIQUE(`inquiry_id`)
);
--> statement-breakpoint
...
ALTER TABLE `inquiries` ADD CONSTRAINT `inquiries_student_id_students_id_fk` FOREIGN KEY (`student_id`) REFERENCES `students`(`id`) ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `school_announcement_dismissals` ADD CONSTRAINT `school_announcement_dismissals_announcement_fk` FOREIGN KEY (`school_announcement_id`) REFERENCES `school_announcements`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `school_announcement_dismissals` ADD CONSTRAINT `school_announcement_dismissals_user_id_users_id_fk` FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
